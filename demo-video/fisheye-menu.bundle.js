/* fisheye-menu.bundle.js — concatenation of fisheye-core.js + fisheye-menu.js
 * Produced from the parent repo. Single non-module script that exposes
 * window.FisheyeMenu.create. Re-bundle via demo-video/bundle.sh if the
 ...
 * @returns {number[]}
 */
function computeFisheyeHeights(n, hoveredIdx, totalHeight, config = DEFAULT_CONFIG) {
  if (n === 0) return [];
 ...
 */
function computeDefaultHeights(n, totalHeight, config = DEFAULT_CONFIG) {
  if (n === 0) return [];
 ...
 */
function cumulativeBoundaries(heights) {
  const boundaries = [0];
 ...
 */
function itemIndexAtY(mouseY, heights) {
  let cumY = 0;
  for (let i = 0; i < heights.length; i++) {
 ...
 */
function create(container, menus, options = {}) {
  ...
  // Public API
  global.FisheyeMenu = { create, computeFisheyeHeights, computeDefaultHeights, pointInTriangle };
})(window);
